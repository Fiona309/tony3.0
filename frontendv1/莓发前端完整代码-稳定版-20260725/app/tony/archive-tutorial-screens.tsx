...
import { API_MODE, createClientId } from './api';
import type {
  AfterVideoTaskData,
  ArchiveDetailData,
  ArchiveSummary,
...
export function ArchiveSavedScreen({
  ...
  return (
    <AppFrame title="保存成功">
      <div className="flex min-h-[calc(100dvh-84px)] flex-col justify-center px-6 pb-12">
...
function archiveStatus(summary: ArchiveSummary) {
  ...
  return { label: '已购买，待开始', action: '开始染发', tone: 'bg-orange-soft text-orange-dark' };
}
...
export function ArchiveDetailScreen({
  ...
  const currentColor =
    ...
    '待确认';
  return (
    <AppFrame title="档案详情" eyebrow={formatDate(archive.created_at)} onBack={onBack}>
      ...
      <BottomBar>
        {archive.completion_record ? (
          ...
          <PrimaryButton onClick={onStartTutorial}>
            {archive.purchase_status === 'saved' ? '查看教程准备' : '开始染发'}
          </PrimaryButton>
        )}
      </BottomBar>
    </AppFrame>
...
function SegmentVideo({
  ...
  useEffect(() => {
    const video = videoRef.current;
    if (!video) return;
    if (autoPlay && video.readyState >= 1) playSegment();
  }, [autoPlay, playSegment, replaySignal, step.step_id]);
  ...
  return (
    <div className="relative aspect-video overflow-hidden rounded-[24px] bg-ink shadow-card">
      {failed ? (
...
function CountdownCard({
  ...
  return (
    <div className="rounded-[24px] border border-sky-dark/30 bg-sky/25 p-4">
...
export function TutorialScreen({
  ...
  onComplete: (qaSummary: string[]) => Promise<void>;
}) {
  ...
  const isLastStep = step.step_no === step.total_steps;
  const showStepDescription = step.source !== 'sensevoice_auto_segment';
  ...
  const handleAudio = async (audio: File) => {
    ...
    window.setTimeout(() => {
      setPhase((current) => (current === 'recognizing' ? 'thinking' : current));
    }, 700);
    try {
  ...
  const startListening = async () => {
    ...
    try {
      ...
      recorder.onstop = () => {
        ...
        const file = new File(
          [blob],
          `voice-${createClientId()}.${extension}`,
          { type: resolvedType },
        );
  ...
  const segmentEnded = () => {
    const prompt =
      isLastStep
        ? {
            text: '最后一步已经播放完了。现在可以继续问我操作问题；如果已经全部完成，请说“结束了”。',
            audio_url: null,
          }
  ...
  const requestFinalVoice = () => {
    if (API_MODE === 'mock') {
      sendMockCommand('finish');
      return;
    }
    setError('可以继续提问；如果已经全部完成，请说“结束了”。');
    void startListening();
  };
  ...
  return (
    ...
    >
      <div className="px-4 pb-6 pt-4">
        {offline ? (
          <StatusNotice tone="warning" title="当前处于离线状态">
            已加载的视频可以继续播放，操作问答将在网络恢复后重试。
          </StatusNotice>
        ) : null}
        <div className={cx(offline && 'mt-3')}>
          <SegmentVideo
            url={session.tutorial_video.url}
            step={step}
            onSegmentEnd={segmentEnded}
            replaySignal={replaySignal}
            autoPlay={!session.awaiting_voice_input || phase === 'video'}
          />
        </div>

        <section className="mt-4 rounded-[24px] border border-line bg-white p-4 shadow-soft">
          <div className="mb-3 flex flex-wrap items-center gap-2 text-[11px] font-bold text-ink-3">
            <span className="rounded-full bg-cream px-2.5 py-1 text-orange-dark">
              {session.tutorial_video.color_name ?? session.tutorial_video.title ?? '当前教程'}
            </span>
            {session.tutorial_video.brand ? (
              <span className="rounded-full bg-cream px-2.5 py-1">
                {session.tutorial_video.brand}
              </span>
            ) : null}
            {session.tutorial_video.tutorial_type ? (
              <span className="rounded-full bg-cream px-2.5 py-1">
                {session.tutorial_video.tutorial_type}
              </span>
            ) : null}
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="text-[10px] font-black uppercase tracking-[.14em] text-orange-dark">
                当前步骤
              </p>
              <h1 className="mt-1 text-xl font-black tracking-tight">{step.title}</h1>
            </div>
            <span className="rounded-full bg-cream px-2.5 py-1 text-[10px] font-bold text-ink-3">
              约 {Math.max(1, Math.round((step.end_time_ms - step.start_time_ms) / 1000))} 秒
            </span>
          </div>
          {showStepDescription ? (
            <p className="mt-3 text-sm leading-6 text-ink-2">{step.description}</p>
          ) : null}
          {step.points?.length ? (
            <ul className="mt-4 space-y-2">
              {step.points.map((point) => (
                <li key={point} className="flex gap-2.5 text-xs leading-5 text-ink">
          ...
          {step.caution ? (
            <div className="mt-4">
              <StatusNotice tone="warning">{step.caution}</StatusNotice>
            </div>
          ) : null}
        </section>
        ...
        {phase !== 'video' ? (
          <section className="mt-4 rounded-[24px] border border-line bg-white p-4 shadow-soft">
            <div className="flex items-center gap-3">
              <span
                className={cx(
                  ...
                  phase === 'listening'
                    ? 'animate-pulse bg-orange text-white'
                    : phase === 'recognizing' ||
                        phase === 'thinking' ||
                        phase === 'prompting'
                      ? 'bg-sky/55 text-ink'
            ...
            {transcript ? (
              <div className="ml-auto mt-4 max-w-[86%] rounded-[17px_17px_4px_17px] bg-ink px-3.5 py-3 text-right text-xs leading-5 text-white">
                {transcript}
              </div>
            ) : null}
            {answer ? (
              <div className="mt-4 rounded-[17px] bg-sky/25 p-3.5">
                <div className="flex items-center gap-2">
                  <SpeakerHigh size={16} weight="fill" />
                  <p className="text-[10px] font-black">莓发回答</p>
                </div>
                <p className="mt-2 text-xs leading-5 text-ink-2">{answer}</p>
                {nextPrompt ? (
                  <p className="mt-2 text-[10px] leading-4 text-ink-3">{nextPrompt}</p>
                ) : null}
              </div>
            ) : null}
            {error ? (
              <p className="mt-3 rounded-[14px] bg-red-50 px-3 py-2 text-xs leading-5 text-red-900">
                {error}
              </p>
            ) : null}
            {API_MODE === 'mock' ? (
              <div className="mt-4 grid grid-cols-3 gap-2">
                <button
                  ...
                  className="tap min-h-11 rounded-[15px] bg-cream px-2 text-[11px] font-black"
                >
                  模拟提问
                </button>
                <button
                  ...
                  disabled={phase === 'recognizing' || phase === 'thinking'}
                  className="tap min-h-11 rounded-[15px] bg-sky/35 px-2 text-[11px] font-black"
                >
                  下一步
                </button>
                <button
                  type="button"
                  onClick={() => sendMockCommand('finish')}
                  disabled={phase === 'recognizing' || phase === 'thinking'}
                  className="tap min-h-11 rounded-[15px] bg-orange-soft px-2 text-[11px] font-black"
                >
                  结束了
                </button>
              </div>
              ...
              <button
                ...
                className="tap mt-4 flex min-h-12 w-full items-center justify-center gap-2 rounded-[16px] bg-orange text-xs font-black text-white disabled:opacity-40"
              >
      ...
      <BottomBar>
        <div className="grid grid-cols-[.8fr_1.35fr] gap-2">
          <SecondaryButton
            onClick={() => {
  ...
  return (
    <AppFrame title="本次染发完成" eyebrow="完成记录">
      ...
      <div className="px-5 pb-6">
        ...
        <section className="mt-4 rounded-[26px] border border-line bg-white p-4 shadow-soft">
          <p className="text-sm font-black">问答总结</p>
...
export function AfterPhotoScreen({
  ...
  return (
    <AppFrame title="生成染发转场视频" eyebrow="可选创作功能" onBack={onBack}>
      {stage === 'intro' ? (
        <>
          <PageIntro
            eyebrow="完全可选"
            title="拍一张现在的头发"
            description="系统会使用预设效果生成短视频。生成失败也不会影响你的完成记录。"
          />
          <div className="px-5">
      ...
      {stage === 'live' ? (
        <div className="relative min-h-[calc(100dvh-84px)] bg-ink">
          <video
            ref={videoRef}
            muted
            playsInline

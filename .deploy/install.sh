...
# 这里刻意不写 MOCK_MODELS / ASR_PROVIDER / VISION_PROVIDER 等业务开关。
# systemd 里 Environment= 会覆盖 EnvironmentFile= 读进来的值，在这儿写一份
# 等于把 .env 架空：线上实际用哪家服务，看的是这个文件而不是 .env，
# 排查时极易被误导。本 unit 只负责「这台机器在哪、绑哪个端口」这类部署事实，
# 业务配置一律以 /opt/meifa/backend/.env 为准。
# 唯一的约束：ASR_PROVIDER 不要设成 sensevoice，那需要 requirements-local-asr.txt
# 里约 2GB 的本地模型依赖，requirements.txt 没装，运行时会报 ImportError。

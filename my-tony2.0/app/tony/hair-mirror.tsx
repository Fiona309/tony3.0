...
void main(){
  ...
  /* ② 次表面散射辉光。黑发把光全吸收，只剩又窄又硬的表面镜面反射；
     漂过的发丝色素没了，光进入内部多次散射再出来，高光又宽又软、边缘外溢。

     必须用 screen 混合，不能用加法。曾经写成 o += tgt*glow，
     那是【加在已经提亮过的画面上且不检查余量】：2 度漂到 8 度时 F=3.81，
     原始明度 0.35 的地方算出 1.252、0.45 的地方 1.597，直接削平成纯白——
     整头像裹了层锡纸。screen 的结果恒在 [0,1] 内，永远不会过曝。

     阈值也改用【提亮后】的宽半径明度：原来拿 wide*F 减固定 0.42，
     F 越大越容易越界，等于提亮倍数越高辉光越失控。 */
  float wx=wide*F;
  float wideL=clamp(wx*(1.0+wx/(W*W))/(1.0+wx),0.,1.);
  float glow=smoothstep(0.62,0.96,wideL)*bloom*m*clamp((F-1.0)*0.5,0.,1.);
  o=1.0-(1.0-o)*(1.0-glow*tgt);
...
/* 1.0 会把高光推到接近纯白，叠上辉光就是金属反光的观感。0.7 保留一点发色 */
const SPEC_KEEP = 0.7;
...
/* 默认 1.0 = 不做发根渐变。立体感应该来自阴影明暗关系，不是人为画一条渐变。
   保留可调，想看布丁头效果可以拖低。 */
const ROOT_KEEP = 1.0;
/* ② 次表面散射辉光强度。0.55 实测严重过曝（像裹锡纸），压到 0.10 起步。
   现在用 screen 混合不会过曝了，但辉光本身就该是很轻的一层。 */
const BLOOM = 0.10;
/* ③ 色度随明度变化的幅度。0 = 整头一个色相（旧行为），1 = 完全按明度在深浅变体间插值 */
const CHROMA_VAR = 0.5;
/* ⑤ 碎发补偿强度 */
const WISP = 0.28;
/* ④ 提亮前的低频降噪强度 */
const DENOISE = 0.35;
